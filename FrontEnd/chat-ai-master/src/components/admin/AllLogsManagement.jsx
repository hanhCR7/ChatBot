...
import { Trash2, Eye, Search, Filter, X, RefreshCw, FileText } from "lucide-react";
...
export default function AllLogsManagement() {
  ...
  // Lấy tất cả logs - sử dụng useCallback để tránh re-render
  const fetchLogs = useCallback(async (page = 1) => {
    setLoading(true);
    const limit = pagination.limit;
    try {
      const data = await getAllLog(page, limit);
      setLogs(data.logs || []);
  ...
  const handleFilter = useCallback(async (page = 1) => {
    ...
    const limit = pagination.limit;
    try {
      const data = await getLogByUserId(filterUserId, page, limit);
      setLogs(data.logs || []);
      const total = data.total || data.logs?.length || 0;
      setTotalLogs(total);
      setTotalPages(Math.ceil(total / limit) || 1);
  ...
  }, [filterUserId, getLogByUserId, fetchLogs, pagination.limit]);
      ...
      >
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
            Xem tất cả các hoạt động của người dùng trong hệ thống
          </p>
        </div>
        <div className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
          <FileText className="w-4 h-4" />
          <span>Tổng: <strong className="text-blue-600 dark:text-blue-400">{totalLogs}</strong> logs</span>
        </div>
      </motion.div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all"
            />
          </div>
          <motion.button
            ...
            disabled={loading}
            className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg font-medium hover:from-blue-700 hover:to-blue-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
          >
                        ...
                        >
                          ...
                          <td className="px-6 py-4">
                            <p className="text-sm text-gray-900 dark:text-gray-100 max-w-md truncate">
                              {log.action || log.message || "Không có mô tả"}
                            </p>
                          </td>
                          <td className="px-6 py-4 whitespace-nowrap">
                            <div className="text-sm text-gray-600 dark:text-gray-400">
                              <div>{dayjs(log.timestamp || log.created_at).format("DD/MM/YYYY HH:mm:ss")}</div>
                        ...
                        <td colSpan={4} className="px-6 py-20 text-center">
                          <div className="flex flex-col items-center gap-3">
                            ...
                            <p className="text-sm text-gray-400 dark:text-gray-500">
                              {filterUserId ? "Thử tìm kiếm với User ID khác" : "Chưa có hoạt động nào được ghi lại"}
                            </p>
                          </div>
                        </td>
                      </motion.tr>
              ...
              <div className="px-6 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                  <p className="text-sm text-gray-600 dark:text-gray-400">
                    ...
                    </strong> trong tổng số <strong className="text-gray-900 dark:text-gray-100">{totalLogs}</strong> logs
                  </p>
            ...
            >
              ...
              <div className="p-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-900 dark:text-gray-100 bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg border border-gray-200 dark:border-gray-600">
                    {selectedLog.action || selectedLog.message || "Không có mô tả"}
                  </p>
                </div>
                <div>
                  <label className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">
                  ...
                  <div className="mt-1 space-y-1">
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">
                      {dayjs(selectedLog.timestamp || selectedLog.created_at).format("DD/MM/YYYY HH:mm:ss")}
                    </p>
                    <p className="text-xs text-gray-500 dark:text-gray-400">
                      {dayjs(selectedLog.timestamp || selectedLog.created_at).fromNow()}
                    </p>
                  </div>
                </div>
              </div>
              <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-end">
                <motion.button
                  ...
                  whileTap={{ scale: 0.95 }}
                  onClick={() => setSelectedLog(null)}
                  className="px-6 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg font-medium hover:from-blue-700 hover:to-blue-800 transition-all"
                >
                  Đóng
                </motion.button>
              </div>
