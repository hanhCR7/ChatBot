...
from schemas import EmailRequest, ActivationEmailRequest, PasswordResetEmail, UserLockNotification, ViolationLockEmailRequest
...
# Gửi mail khi thấy user vi phạm nhiều lần
@router.post("/send-violation-lock-email/", status_code=status.HTTP_200_OK)
async def send_violation_lock_email_api(request: ViolationLockEmailRequest, db: Session = Depends(get_db)):
    """Gửi mail khi user bị khóa do vi phạm nội dung"""
    email_body = f"""
    ...
    <p>Thời gian khóa: <strong>{request.duration}</strong>.</p>
    <p>Nếu bạn nghĩ đây là nhầm lẫn, hãy liên hệ quản trị viên để được hỗ trợ.</p>
    """
    await send_email(db, request.recipient, "Tài khoản bị tạm khóa do vi phạm", email_body)
    return {
        "status": "success",
        "message": "Email gửi thành công"
    }